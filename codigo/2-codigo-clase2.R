...
casen_2017 <- readRDS("datos/2-casen-pobreza.rds") # Alternativa ante no lectura SPSS
...
casen_pond <- casen_2017  %>% as_survey_design(ids = varunit, strata = varstrat, weights = expr)
...
# Estimación de punto ponderada 19,828% (*)- Intervalo de confianza --> Li: 19,159% - Ls: 20,497%
...
casen_pond2 <- casen_2017 %>% as_survey_design(ids = varunit, weights = expr)
...
pobrezamulti2 <- casen_pond2 %>% 
  ...
  summarize(frecuencias = survey_total(),
            ...
            n = unweighted(n()))
# Pobres multidimensionales
# Estimación de punto ponderada 19,828% (*)- Intervalo de confianza --> Li: 19,027% - Ls: 20,629%
### INTERVALOS CALCULADOS SON DIFERENTES


# ----- 4. CONSTRUIR BASE PARA UTILIZAR OFF LINE

casen_seleccion <- select(casen_2017, pobreza_multi_5d, expr, expc, varunit, varstrat, region, comuna)
...
saveRDS(casen_seleccion, file = "datos/2-casen-pobreza.rds")
